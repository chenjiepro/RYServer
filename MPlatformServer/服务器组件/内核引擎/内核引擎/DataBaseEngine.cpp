...
bool CDataBase::SetConnectionInfo(LPCTSTR dwDBAddr, WORD wPort, LPCTSTR szDBName, LPCTSTR szUser, LPCTSTR szPassword)
{
	...
	m_strConnect.Format(TEXT("Provider=SQLOLEDB.1;Password=%s;Persist Security Info=True;User ID=%s;Initial Catalog=%s;Data Source=%s,%ld;"),
		strPassword, strUser, strDBName, dwDBAddr, wPort);
...
//插入参数
VOID CDataBase::AddParameter(LPCTSTR pszName, DataTypeEnum Type, ParameterDirectionEnum Direction, LONG Size, CDBVarValue & DBVarValue)
{
	try
	{
		_ParameterPtr Parameter = m_DBCommand->CreateParameter(pszName, Type, Direction, Size, DBVarValue);
...
bool CDataBase::IsConnectError()
{
	try
	{
		...
		//参数判断
		if (m_DBConnection->Errors->Count > 0)
		{
			//变量定义
...
//请求事件
bool CDataBaseEngine::PostDataBaseRequest(WORD wRequestID, WORD dwContextID, VOID * pData, WORD wDataSize)
{
	//校验参数
...
//延期请求
bool CDataBaseEngine::DeferDataBaseRequest(WORD wRequestID, WORD dwContextID, VOID * pData, WORD wDataSize)
{
	////缓冲锁定
	//CWHDataLocker ThreadLock(m_CriticalLocker);
		...
		case EVENT_CONTROL:
			{
				...
				{
					ASSERT(m_pIDataBaseEngineSink != NULL);
					return m_pIDataBaseEngineSink->OnDataBaseEngineControl(pControlEvent->wControlID, pData, wDataSize);
				}
				catch (...)
				{
					ASSERT(FALSE);
