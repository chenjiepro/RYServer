...
bool CTraceServiceControl::TraceString(LPCTSTR pszString, enTraceLevel TraceLevel)
{
	...
	_sntprintf(TraceInfo.szString, CountArray(TraceInfo.szString), TEXT("%s\n"), pszString);

	//插入队列
	CWHDataLocker	ThreadLock(m_CriticalSection);
	WORD wDataSize = sizeof(TraceInfo) - sizeof(TraceInfo.szString) + CountStringBuffer(TraceInfo.szString));
