...
private:
	...
	//拷贝函数
	VOID MD5Memcopy(BYTE * pcbOutput, BYTE * pcbInput, UINT nLen);
	//转换函数
...
VOID CMD5Aide::MD5Update(BYTE * pcbInput, UINT inputLen)
{
	...
	{
		MD5Memcopy((BYTE*)&m_cbBuffer[nIndex], (BYTE *)pcbInput, nPartLen);
		MD5Transform(m_lState, m_cbBuffer);
		for (i = nPartLen; i + 63<inputLen; i += 64) MD5Transform(m_lState, &pcbInput[i]);
		nIndex = 0;
	}
	else i = 0;
	MD5Memcopy((BYTE*)&m_cbBuffer[nIndex], (BYTE *)&pcbInput[i], inputLen - i);
...
//拷贝函数
VOID CMD5Aide::MD5Memcopy(BYTE * pcbOutput, BYTE * pcbInput, UINT nLen)
{
	for (UINT i = 0; i < nLen; ++i)
	{
...
{
	ULONG a = lState[0], b = lState[1], c = lState[2], d = lState[3];
	ULONG x[16];
...
//生成密文
bool CWHEncrypt::XorEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncryptData, WORD wMaxCount)
{
	//变量定义
	...
	lstrcpyn(pszEncryptData, strEncrypData, wMaxCount);

	return true;
}
//解开密文
bool CWHEncrypt::XorCrevasse(LPCTSTR pszEncryptData, LPTSTR pszSourceData, WORD wMaxCount)
{
	//设置结果
	pszSourceData[0] = 0;

	//变量定义
	CT2CW strEncrypData(pszEncryptData);
...
//生成密文
bool CWHEncrypt::MapEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncryptData, WORD wMaxCount)
{
	//效验参数
	ASSERT(wMaxCount>lstrlen(pszEncryptData));
	ASSERT((pszEncryptData != NULL) && (pszSourceData != NULL));

	//变量定义
	INT nLength = lstrlen(pszSourceData);
	BYTE * pcbEncryptData = (BYTE *)pszEncryptData;
	...
	for (UINT i = 0; i<nLength*sizeof(TCHAR); i++)
	{
		...
		pcbEncryptData[i] = g_cbEncryptMap[cbIndex];
	}

	//设置结果
	pszEncryptData[nLength] = 0;

	return true;
}
//解开密文
bool CWHEncrypt::MapCrevasse(LPCTSTR pszEncryptData, LPTSTR pszSourceData, WORD wMaxCount)
{
	//效验参数
	ASSERT(wMaxCount>lstrlen(pszEncryptData));
	ASSERT((pszEncryptData != NULL) && (pszSourceData != NULL));
	...
	//变量定义
	INT nLength = lstrlen(pszEncryptData);
	BYTE * pcbEncryptData = (BYTE *)pszEncryptData;
	BYTE * pcbSourceData = (BYTE *)pszSourceData;
